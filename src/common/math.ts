...
export function activeElapsedPctOf(
	...
	const totalActive = activeHoursBetween(
		periodStart,
		resetsAt,
		activeStartH,
		activeEndH,
	...
	const elapsed = activeHoursBetween(
		periodStart,
		clampedNow,
		activeStartH,
		activeEndH,
...
export function frozenExpectedPctOf(
	...
	// Snap to the most recent end-of-active-window that has already passed.
	// Called during bonus/sleep to show the "expected" marker frozen at end of today's active hours.
	const eod = new Date(nowMs);
	eod.setHours(activeEndH, 0, 0, 0);
	if (eod.getTime() > nowMs) eod.setDate(eod.getDate() - 1);
	return activeElapsedPctOf(
		eod.getTime(),
		resetsAt,
		periodMs,
		activeStartH,
