...
import {
  elapsedPctOf, activeElapsedPctOf, timeWindowOf,
  frozenExpectedPctOf, todayEndExpectedPctOf, deltaPpOf, severityOf,
} from './math.js';
import { buildSignals, classifySituation } from './signals.js';
import { applyBarGradient } from './ui/components/bar.js';
import { ensureMarker, MARKER_CLASS } from './ui/components/now-marker.js';
import { ensureDayDividers } from './ui/components/day-dividers.js';
import {
  ensurePill, PILL_CLASS, PILL_OVER, PILL_UNDER, PILL_NEUTRAL,
  SUPPRESS_PILL_BEFORE_MS, dirIconName, sevStyle,
...
import { ensureLucide, makeLucideIcon, renderLucideIcons, isLucideReady } from './ui/lucide.js';
...
function appendStat(target, iconName, value) {
  if (isLucideReady()) { target.appendChild(makeLucideIcon(iconName, 12)); renderLucideIcons(target); }
...
function renderMarkerAndPill(bucketKey, util, resetsAt, cfg) {
  ...
  const elapsedPct = bucketKey === 'five_hour'
    ? elapsedPctOf(now, resetsAtMs, periodMs)
    : activeElapsedPctOf(now, resetsAtMs, periodMs, cfg.activeStartH, cfg.activeEndH);
  ...
  const markerPct = bucketKey === 'five_hour'
    ? elapsedPct
    : (win === 'active')
      ? elapsedPct
      : frozenExpectedPctOf(now, resetsAtMs, periodMs, cfg.activeStartH, cfg.activeEndH);
  ...
  if (bucketKey === 'five_hour') {
    ...
    Object.assign(lh.style, {
      ...
      padding: '2px 7px', background: ws.bg, color: ws.color,
      borderTop: `1px solid ${ws.borderC}`, borderBottom: `1px solid ${ws.borderC}`,
      borderLeft: `1px solid ${ws.borderC}`, borderRight: '1px solid rgba(0,0,0,0.12)',
    ...
    Object.assign(rh.style, {
      display: 'inline-flex', alignItems: 'center', gap: '4px',
      padding: '2px 7px', background: ts.bg, color: ts.color,
      borderTop: `1px solid ${ts.borderC}`, borderBottom: `1px solid ${ts.borderC}`,
...
export function renderAllMarkers(json, cfg, attempt = 0) {
  ...
  for (const key of Object.keys(BUCKET_MAP)) {
    ...
    if (!bucket || bucket.utilization == null || bucket.resets_at == null) continue;
    if (renderMarkerAndPill(key, bucket.utilization, bucket.resets_at, cfg)) renderedAny = true;
  }
