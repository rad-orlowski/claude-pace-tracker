...
export const MASK_CLASS = "__claude-pace-mask";

export function buildBarGradient(
	periodStartMs,
	periodMs,
...
export function buildWeeklyBar(days = 7) {
	...
	return `linear-gradient(to right,${stops.join(",")})`.replace(
		"100.000%)",
		"100%)",
	);
}

export function applyBarGradient(
	bar,
	periodStartMs,
	periodMs,
	usedPct,
	...
	// Mask covers the remaining (unused) portion of the bar.
	// When essentially nothing is used the mask spans the full bar — round all corners.
	// Otherwise only round the right edge (the left edge is a clean cut at the usage boundary).
